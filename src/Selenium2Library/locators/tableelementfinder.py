...
class TableElementFinder(ContextAware):
    ...
    def find_by_col(self, table_locator, col, content):
        ...
        return self._search_in_locators(locators, content)
    """ Added By Yuhui"""
    def find_in_col(self,  table_locator, col, content):
        locators = self._parse_table_locator(table_locator, 'col')
        locators = [locator % str(col) for locator in locators]
        return self._search_index_in_locators( locators, content)
    ...
    def _search_in_locators(self, locators, content):
        ...
        return None
    """ Added By Yuhui Xu"""
    def _search_index_in_locators(self, locators, content):        
        for locator in locators:
            index = 0
            elements = self._element_finder.find(self.browser, locator)
